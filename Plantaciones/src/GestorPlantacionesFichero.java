...
public class GestorPlantacionesFichero implements IGestorPlantaciones{
	...
	public GestorPlantacionesFichero(){
		leerParcelas();
	}
	...
	public void borrarPlantacion(Plantacion p){
		System.out.println("dentro delplanta -> " + p.toString());
		try {
			File fichero_plantaciones = new File(ARCHIVO);
			BufferedReader br_fichero_plantaciones = new BufferedReader(new FileReader(fichero_plantaciones));
			ArrayList<String> lectura_fichero_completa = new ArrayList<>();

			String linea_leida = br_fichero_plantaciones.readLine();

			while (linea_leida != null) {
				if (!linea_leida.startsWith(p.toString()))
					lectura_fichero_completa.add(linea_leida);
				linea_leida = br_fichero_plantaciones.readLine();
			}

			fichero_plantaciones.delete();
			br_fichero_plantaciones.close();

			fichero_plantaciones.createNewFile();
			BufferedWriter bw_fichero_plantaciones = new BufferedWriter(new FileWriter(fichero_plantaciones));

			for (String linea_guardada : lectura_fichero_completa) {
				System.out.println("Linea es::::::" + linea_guardada);
				bw_fichero_plantaciones.write(linea_guardada + "\r\n");
			}

			bw_fichero_plantaciones.close();
			
			// Actualizar listado
			frmPrincipal.actualizarListado(new ArrayList<>());

		} catch (Exception ex) {
			System.out.println("Excepcion: " + ex);
		}
		System.out.println("Eliminando parcela seleccionada...");
